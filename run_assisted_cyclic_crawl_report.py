#!/usr/bin/env python3
"""
Instrumentation/report runner for the current assisted cyclic crawl baseline.
...
class MetricsRecorder:
    ...
    def _build_summary(self, trial_name: str):
        ...
        summary = {
            "trial_name": trial_name,
            "type": "assisted_cyclic_crawl_report",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Assisted cyclic crawl report runner")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results") / "assisted_report")
...
def run(args):
    ...
    print("\nStarting assisted cyclic crawl report run")
    print("-" * 72)
    print(f"Scene:       {scene}")
