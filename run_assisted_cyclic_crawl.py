#!/usr/bin/env python3
"""
Highly assisted cyclic ladder crawler for the modified Unitree Go2.
...
3. temporarily disable collision for the moving gripper;
4. open the moving gripper kinematically;
5. move its palm through lift -> next-rung-above -> next-rung-grasp waypoints;
6. restore collision;
7. close to the measured successful grasp pose.
...
class AssistedCyclicCrawler:
    ...
    def _begin_leg_step(self):
        ...
        translation = target_row[3] - source_row[3]
        ...
        lift_palm = self.start_palm + np.array([0.0, 0.0, self.lift_height])
        transfer_palm = lift_palm + translation
        self.goal_palm = self.start_palm + translation

        reference = d.qpos.copy()
        self.q_lift, r1 = solve_leg_ik(
            self.model, reference, c, leg=leg,
            target_palm=lift_palm, initial_leg_qpos=self.q_start
        )
        reference[c._leg_qpos_adrs[leg]] = self.q_lift
        self.q_transfer, r2 = solve_leg_ik(
            self.model, reference, c, leg=leg,
            target_palm=transfer_palm, initial_leg_qpos=self.q_lift
        )
        reference[c._leg_qpos_adrs[leg]] = self.q_transfer
        self.q_goal, r3 = solve_leg_ik(
            self.model, reference, c, leg=leg,
            target_palm=self.goal_palm, initial_leg_qpos=self.q_transfer
        )
        worst = max(r1, r2, r3)
        if worst > 0.02:
            raise RuntimeError(
                f"{leg} assisted waypoints not reachable enough; worst residual={worst:.4f} m"
            )

        self._disable_leg_collision(leg)
    ...
    def apply(self):
        ...
        if elapsed < t1:
            ...
            self.phase = f"LIFT_{leg}"
            qleg = interpolate(self.q_start, self.q_lift, (elapsed - t1) / self.lift_time)
            qfinger = c.open_finger_pose
        elif elapsed < t3:
            self.phase = f"TRANSFER_{leg}"
            qleg = interpolate(
                self.q_lift, self.q_transfer, (elapsed - t2) / self.transfer_time
            )
            qfinger = c.open_finger_pose
        elif elapsed < t4:
            self.phase = f"LOWER_{leg}"
            qleg = interpolate(
                self.q_transfer, self.q_goal, (elapsed - t3) / self.lower_time
            )
            qfinger = c.open_finger_pose
        elif elapsed < t5:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Assisted cyclic ladder crawler")
...
def run(args):
    ...
    def step_once():
        ...
        if not crawl.started:
            ...
            # Use the public GraspStatus string.  The controller's internal
            # _state is a GraspState Enum, so comparing it directly with the
            # string "HOLDING" never succeeds.
            ...
            if status.state == "HOLDING":
                print(
                    ...
                    "Starting assisted cyclic crawl."
                )
                crawl.begin()
        else:
            crawl.apply()
