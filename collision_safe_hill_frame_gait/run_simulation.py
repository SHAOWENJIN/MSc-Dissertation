...
Important:
- The robot is free-floating; the space-structure truss is fixed to the world.
- Runtime motion is produced by clipped motor torques with collision enabled.
- Each idealized mechanical gripper latch engages only after physical rung
  contact is detected, and releases before that gripper moves.
- The path is planned and trajectory-assisted, so this is not evidence of an
  autonomous climbing controller.
- Mechanical work / actuator power are outside this report's current scope.
"""
...
# Keep this student-facing package isolated from similarly named files in the
# repository root.  Running this script always imports the controller and gait
# implementation shipped beside it.
...
PHASE_ORDER = [
    ...
    "OPEN_FL", "WITHDRAW_FL", "TRANSFER_FL", "APPROACH_FL", "ESCAPE_FL", "CLOSE_FL", "HOLD_FL",
    "OPEN_FR", "WITHDRAW_FR", "TRANSFER_FR", "APPROACH_FR", "ESCAPE_FR", "CLOSE_FR", "HOLD_FR",
    "OPEN_RL", "WITHDRAW_RL", "TRANSFER_RL", "APPROACH_RL", "ESCAPE_RL", "CLOSE_RL", "HOLD_RL",
    "OPEN_RR", "WITHDRAW_RR", "TRANSFER_RR", "APPROACH_RR", "ESCAPE_RR", "CLOSE_RR", "HOLD_RR",
    "BODY_SHIFT",
    "COMPLETE",
]
...
class MetricsRecorder:
    ...
    def write_outputs(self, trial_name: str):
        ...
        summary_lines = [
            ...
            f"Final physical contacts: {summary['final_physical_contact_count']}",
            f"Final geometric wraps: {summary['final_geometric_wraps']}",
            f"Final active latches: {summary['final_active_latches']}",
            f"Recorded duration: {summary['recorded_duration_s']:.3f} s",
            "",
            "Important note:",
            "  This is a motor-driven, trajectory-assisted gait baseline.",
            "  The robot is free; the space-structure truss is fixed.",
            "  Idealized gripper latches engage only after verified contact.",
            "  It is not an autonomous climbing validation run.",
    ...
    def _build_summary(self, trial_name: str):
        ...
        final_contacts = self.controller._detect_contacts(self.model, self.data)
        final_wraps = sorted(
            final_contacts.get("geometrically_wrapped_grippers", set())
        )
        final_latches = sorted(
            leg
            for leg, weld_id in self.controller._grasp_weld_ids.items()
            if weld_id >= 0 and bool(self.data.eq_active[weld_id])
        )
        ...
        summary = {
            ...
            "final_physical_contact_count": int(
                final_contacts.get("physical_count", 0)
            ),
            "final_geometric_wraps": final_wraps,
            "final_active_latches": final_latches,
...
def parse_args():
    ...
    parser.add_argument(
        "--gravity-scale",
        type=float,
        choices=(0.0, 0.8, 0.7, 0.6, 0.5),
        default=0.0,
        help="Uniform gravity scale; 0.0 is the Hill-frame microgravity approximation.",
    )
    parser.add_argument(
        "--sequence",
        default="RR,RL,FR,FL",
        help="Unique leg names. The default runs one full four-leg crawl and body shift.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PACKAGE_ROOT / "results" / "experiment_4_hill_frame",
    )
...
def run(args):
    ...
    if args.headless:
        ...
        with mj_viewer.launch_passive(model, data) as viewer:
            # A fixed model camera disables the normal mouse navigation. Start
            # with a free camera so orbit, pan and zoom work immediately.
    ...
    # Ensure the report contains the terminal COMPLETE state even when the
    # normal sample interval has not elapsed since the last gait update.
    recorder.next_sample_time = -float("inf")
    recorder.maybe_record()
